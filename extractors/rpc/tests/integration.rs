...
async fn test_integration_metrics_server_basic() {
    ...
    // Verify that the metrics endpoint contains expected metric names
    assert!(
        metrics_text.contains("rpcextractor_rpc_fetch_duration_seconds")
            || metrics_text.contains("# HELP")
            || metrics_text.contains("process_"),
        "Metrics response should contain metric definitions"
    );
